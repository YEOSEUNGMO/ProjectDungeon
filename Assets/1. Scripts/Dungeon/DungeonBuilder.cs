...
public class DungeonBuilder : SingletonMonobehaviour<DungeonBuilder>
{
    ...
    public bool GenerateDungeon(DungeonLevelSO currentDungeonLevel)
    {
        ...
        while (!dungeonBuildSuccessful && dungeonBuildAttempts < Settings.maxDungeonRebuildAttempts)
        {
            ...
            //Loop until dungeon successfully build or bore than max attempts for node graph
    ...
    private void LoadRoomTemplatesIntoDictionary()
    {
        ...
        foreach (RoomTemplateSO roomTemplate in roomTemplateList)
        {
            ...
            {
                Debug.LogWarning("Duplicate Room Template Key In " + roomTemplateList);
            }
        }
    }
    ...
    private RoomNodeGraphSO SelectRandomRoomNodeGraph(List<RoomNodeGraphSO> roomNodeGraphList)
    {
        ...
        {
            Debug.LogError("No room node graphs in list");
            return null;
        }
    }
    ...
    /// <summary>
    /// Atetmpt to randomly build the dungeon for the specified room nodeGraph.
    /// 
    /// </summary>
    /// <param name="roomNodeGraph"></param>
    /// <returns>ture : if a successful random layout was genrated
    /// false : if a problem was encoutred and anothre attempt is required</returns>
    private bool AttemptToBuildRandomDungeon(RoomNodeGraphSO roomNodeGraph)
    {
        //Creat Open Room NOde Queue
        ...
        bool noRoomOVeralps = true;

        // Process open room nodes queue
        noRoomOVeralps = ProcessRoomsInOpenRoomNodeQueue(roomNodeGraph, openRoomNodeQueue, noRoomOVeralps);

        // If all the room nodes have been processed and there hasn't been a room overlap then return true
        if (openRoomNodeQueue.Count == 0 && noRoomOVeralps)
        {
            return true;
        }
        else
    ...
    /// </summary>
    private bool ProcessRoomsInOpenRoomNodeQueue(RoomNodeGraphSO roomNodeGraph, Queue<RoomNodeSO> openRoomNodeQueue, bool noRoomOVeralps)
    {

        //while room node in open room node queue & no room overlaps detected.
        while (openRoomNodeQueue.Count > 0 && noRoomOVeralps == true)
        {
            ...
            else    //the room type isn't an entrance
            {
                // Get parent room for node
                Room parentRoom = dungeonBuilderRoomDictionary[roomNode.parentRoomNodeIDList[0]];

                // See if room can be placed without overlaps
                noRoomOVeralps = CanPlaceRoomWithNoOverlaps(roomNode, parentRoom);
            }
        }

        return noRoomOVeralps;
    }
    ...
    {
        // initailize and assume overlap until proven otherwise
        ...
        while (roomOverlaps)
        {
            ...
            // Get a random room template for room node that is consistent with the parent door orientaion
    ...
    private bool PlaceTheRoom(Room parentRoom, Doorway doorwayParent, Room room)
    {
        ...
        //Caculate adjustment position offset based on room doorway position that we are trying to connect
        ...
        //Caculate room lower bounds and upper bounds based on positioning to align with parent doorway
    ...
    private Room CheckForRoomOverlap(Room roomToTest)
    {
        ...
        foreach (var pair in dungeonBuilderRoomDictionary)
        {
    ...
    /// <summary>
    /// Get a random room template from the roomtemplatlist that matches the roomType and return it
    ...
    /// <summary>
    /// Creat deep copy of doorway list
    /// </summary>
    ...
    /// <summary>
    /// Creat deep copy of string list
    /// </summary>
